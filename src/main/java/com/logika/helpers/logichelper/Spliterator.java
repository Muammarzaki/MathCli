...
public class Spliterator {
     ...
     * @return
     */
    public static List<Boolean> Spliters(String v) {
        Spliterator spliterator = new Spliterator();
        v.trim();
        if (v.contains("~") && v.substring(v.indexOf("~") + 1, v.indexOf("~") + 2).equals("(") && v.length() > 0) {
            return Negasi.negasi(spliterator.findMethod(spliterator.getbetween(v)));
        } else {
            return spliterator.findMethod(spliterator.getbetween(v));
        }
